...
def _by_status(db: Session, status: models.InterviewStatus, skip: int, limit: int):
    return (
        db.query(models.InterviewSchedule)
        .filter(models.InterviewSchedule.status == status)
        .order_by(
            models.InterviewSchedule.scheduled_date.desc().nullslast(),
...
def list_scheduled(db: Session = Depends(get_db),
                   skip: int = Query(0, ge=0),
                   limit: int = Query(500, ge=1, le=2000)):
    return _by_status(db, models.InterviewStatus.Scheduled, skip, limit)
...
def list_completed(db: Session = Depends(get_db),
                   skip: int = Query(0, ge=0),
                   limit: int = Query(500, ge=1, le=2000)):
    return _by_status(db, models.InterviewStatus.Completed, skip, limit)
...
def list_in_progress(db: Session = Depends(get_db),
                     skip: int = Query(0, ge=0),
                     limit: int = Query(500, ge=1, le=2000)):
    return _by_status(db, models.InterviewStatus.InProgress, skip, limit)