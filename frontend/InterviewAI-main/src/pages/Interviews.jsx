...
const aggregateUpcomingInterviews = () => {
  const aggregated = {};
  
  upcomingInterviewsData.forEach(interview => {
...
export default function Interviews() {
  ...
  const upcomingInterviews = aggregateUpcomingInterviews();
  ...
  const stats = {
    total: dummyInterviews.length,
    completed: dummyInterviews.filter(i => i.status === "Completed").length,
    escalated: dummyInterviews.filter(i => i.status === "Escalated").length,
    scheduled: dummyInterviews.filter(i => i.status === "Scheduled").length,
    inProgress: dummyInterviews.filter(i => i.status === "In Progress").length,
  };
                ...
                <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 600 }}>
                  ...
                  <tbody>
                    {filteredInterviews.map((interview, index) => {
                      const Status = STATUS_PILL[interview.status];
                      return (
                        <tr 
                          key={interview.id} 
                          style={{ 
